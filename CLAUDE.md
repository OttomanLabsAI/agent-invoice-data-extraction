...
wrangler.jsonc        main src/index.js, assets ./public (binding ASSETS), D1 DB "invoice-agent", R2 FILES (no bucket_name - see below), cron */5
```
...
- **Provisioning on deploy:** wrangler creates a D1 database for a binding that has `database_name` but no `database_id`, and creates an R2 bucket only for a binding with no `bucket_name` at all (named `<worker>-<binding>`, so `agent-invoice-data-extraction-files`). Naming the bucket in the config means "bind to an existing bucket", and the deploy fails with "R2 bucket not found" if it does not exist. Keep the R2 binding nameless unless the bucket is created by hand first. R2 must be enabled on the account once.
...
Not yet exercised end to end: the deploy on the owner's Cloudflare account (the first attempt provisioned the D1 database by name and failed on the named R2 bucket, hence the nameless binding; if a build still cannot create the bucket, enable R2 on the account or create it by hand and name it in the config), Gmail OAuth with a Web-application client on the deployed host, Claude extraction on a real invoice (fixture-only in tests - run the sample PDF through *Process upload* with a live key first), and the Intacct push - written to the XML gateway spec, never sent to a live company. First real post should be `Draft`, compared against a hand-keyed bill; expect to adjust `TAXSOLUTIONID` / tax detail names / whether PO matching should go through Purchasing rather than `DOCNUMBER`.
