...
agent/extractor.py      Claude Messages calls: classify_email (label_email tool) and extract_invoice (record_invoice tool) -> normalise(); system prompt = template (shipped or edited) + reference section
...
agent/prompts.py        CLASSIFY_SYSTEM / EXTRACT_SYSTEM (the shipped system prompts with {company_name} / {default_currency}), fill(), is_default()
...
- **Reference text (RAG):** `rag.retrieve(text, query)` - the whole text when it is under `RAG_MAX_CHARS` (16,000), otherwise the paragraphs (blank-line separated) that share words with the email, in original order, within the budget. The query is sender + subject + attachment names + body. The text goes into the system prompt through `_reference_section`. Both tabs show `rag.describe()`. The text starts empty by design (the owner asked for it empty, with examples of what to add listed on the tab in `templates/agent_*.html`).
- **System prompts** are editable: `settings["classify_system_prompt"]` / `extract_system_prompt` hold the template, shipped as `prompts.CLASSIFY_SYSTEM` / `EXTRACT_SYSTEM`. `extractor.classification_system_prompt(company, reference, template)` and `system_prompt(company, currency, reference, template)` fill `{company_name}` / `{default_currency}` by plain replacement (`prompts.fill`, so other braces survive) and append the reference section; a blank template means the shipped one. `pipeline` passes the settings' templates as `system_template`. On each tab: *Restore default* posts `action=reset_prompt` (disabled while `prompts.is_default()`), saving an empty box also restores the default with a warning flash, and *What the AI receives* renders the assembled system prompt for the saved settings plus the per-email note for `app.SAMPLE_EMAIL` (`extractor.classification_context` / `email_context`). Prompt wording changes go in `prompts.py`, not `extractor.py`.
